...
pub(crate) async fn push_inner_with_registry<R: Registry>(
    ...
    let empty_config_digest = Digest::from_bytes_sha256(EMPTY_CONFIG_BYTES);
    if !registry
        .head_blob(&target.registry, &target.repo, &empty_config_digest)
        .await
        .map_err(|e| anyhow!("head_blob empty-config {empty_config_digest}: {e}"))?
    {
        let bytes = Bytes::from_static(EMPTY_CONFIG_BYTES);
        ...
        registry
            ...
            .await
            .map_err(|e| anyhow!("push_blob_stream empty-config {empty_config_digest}: {e}"))?;
    }
...
mod tests {
    ...
    use bytes::Bytes;
    use pichi_artifact::{
        EmptyConfigDescriptor, Layer, MEDIA_TYPE_PICHI_ARTIFACT_V1, ScuteAnnotations,
        ScuteDescriptor,
    };
