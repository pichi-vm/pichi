...
use pichi_artifact::{
    Digest, Layer, MEDIA_TYPE_PICHI_ARTIFACT_V1, MEDIA_TYPE_PICHI_PMI_V1, Manifest, Reference,
    ReferenceKind,
};
...
fn build_sidecar(digest: &Digest, m: &Manifest) -> PichiSidecar {
    ...
    for layer in &m.layers {
        let media = match layer {
            Layer::Scute(_) => "application/vnd.pichi.scute.v1",
            Layer::ScuteZstd(_) => "application/vnd.pichi.scute.v1+zstd",
            Layer::Pmi(_) => MEDIA_TYPE_PICHI_PMI_V1,
        };
        if matches!(layer, Layer::Pmi(_)) {
            pmi_present = true;
        } else {
            scute_count += 1;
        }
