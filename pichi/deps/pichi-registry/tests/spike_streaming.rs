...
fn spike_a5_zstd_write_decoder_sink_shape() {
    ...
    let compressed = zstd::encode_all(&original[..], 0).expect("zstd encode");

    // Now decode by writing the compressed bytes into a write::Decoder
    // whose inner sink is a Vec<u8>.
    let mut sink: Vec<u8> = Vec::new();
    {
        let mut decoder = zstd::stream::write::Decoder::new(&mut sink).expect("decoder");
        decoder.write_all(&compressed).expect("feed compressed");
        decoder.flush().expect("flush");
    } // decoder dropped — final block flushed
    assert_eq!(sink, original, "decoded bytes must match original");
}
