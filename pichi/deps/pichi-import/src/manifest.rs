...
use pichi_artifact::{
    Digest, EmptyConfigDescriptor, Layer, MEDIA_TYPE_PICHI_ARTIFACT_V1, Manifest, ScuteAnnotations,
    ScuteDescriptor,
};
...
pub(crate) fn build(
    ...
    // Append PMI layer when present (appliance form). Order is not load-bearing
    // per Phase 42 D-03; appending after Scute matches test expectations.
...
mod tests {
    ...
    fn manifest_has_locked_chain_annotations() {
        ...
        let salt = vec![0u8; 32];
        let m = build(&cow_digest, 16384, &salt, "2026-05-07T12:00:00Z", None).unwrap();
        assert_eq!(
            m.annotations
