...
//!
//! Public modules `cow` and `verity` implement the dm-snapshot persistent
//! COW writer and the dm-verity v1 hash tree builder respectively.
//! `verity` is `pub` so Phase 44 (`pichi pull`) can call into it without
//! a `tools/import` build dependency cycle (per CONTEXT D-03).
...
/// IMPORT-07 upper bound (CONTEXT "Deferred Ideas"): 2048 sectors = 1 MiB.
/// Lower bound is enforced inside `cow::validate_chunk_size`.
pub const MAX_CHUNK_SIZE_SECTORS: u32 = 2048;
...
pub struct ImportArgs {
    ...
    /// `None` = use default of 32 sectors (16 KiB) per
    /// `DM_CHUNK_SIZE_DEFAULT_SECTORS`.
    pub chunk_size_sectors: Option<u32>,
...
pub fn run(args: ImportArgs, graphroot: &Path) -> Result<()> {
    // Validate chunk size (lower bound in cow::, upper bound here).
    let chunk_size_sectors = args
        .chunk_size_sectors
        .unwrap_or(cow::DEFAULT_CHUNK_SIZE_SECTORS);
    cow::validate_chunk_size(chunk_size_sectors).context("invalid --chunk-size")?;
    if chunk_size_sectors > MAX_CHUNK_SIZE_SECTORS {
        bail!(
            "--chunk-size must be <= {MAX_CHUNK_SIZE_SECTORS} sectors (1 MiB) \
    ...
    // Step 5b: build manifest (with optional PMI layer reference).
    let pmi_layer_for_manifest = pmi_staged.as_ref().map(|s| s.layer.clone());
    let pichi_manifest = manifest::build(
        &cow_digest,
        cow_size,
        &full_salt,
        &args.created_rfc3339,
        pmi_layer_for_manifest,
    )
...
mod tests {
    ...
    #[test]
    fn rejects_chunk_size_below_min() {
        let tmp = TempDir::new().unwrap();
        let raw = tmp.path().join("input.raw");
        std::fs::write(&raw, b"\x00").unwrap();
        let (mut args, graphroot) = make_args(&tmp, raw, "x:y");
        args.chunk_size_sectors = Some(4);
        let err = run(args, &graphroot).unwrap_err();
    ...
    fn rejects_chunk_size_above_max() {
        ...
        std::fs::write(&raw, b"\x00").unwrap();
        let (mut args, graphroot) = make_args(&tmp, raw, "x:y");
        args.chunk_size_sectors = Some(MAX_CHUNK_SIZE_SECTORS + 2);
        let err = run(args, &graphroot).unwrap_err();
        assert!(format!("{err:#}").contains("chunk-size"), "{err:#}");
    }

    #[test]
    fn rejects_chunk_size_not_power_of_two() {
        let tmp = TempDir::new().unwrap();
        let raw = tmp.path().join("input.raw");
        std::fs::write(&raw, b"\x00").unwrap();
        let (mut args, graphroot) = make_args(&tmp, raw, "x:y");
        args.chunk_size_sectors = Some(33);
        let err = run(args, &graphroot).unwrap_err();
        assert!(format!("{err:#}").contains("chunk-size"), "{err:#}");
    }
}
