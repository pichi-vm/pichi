//! PMI parser: PE + CBOR + defensive validation.
//!
//! Single public entry point: [`parse`]. The implementation walks the
//! PE once via the `object` crate, runs each defensive check from
//! `dillo/ARCHITECTURE.md` §5, then returns a typed [`ParsedPmi`].
...
pub fn parse(bytes: &[u8], opts: &ParseOptions) -> Result<ParsedPmi, Error> {
    ...
    let (actions, active_section_names, fill_kinds) = resolve_actions(&actions_raw, &raw_sections)?;
...
fn spread_check(ranges: &[(u64, u64)], total_loaded: u64, memory_cap: u64) -> Result<(), Error> {
    if ranges.is_empty() {
        return Ok(());
    }
