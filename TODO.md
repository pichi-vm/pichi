...
Audit fix 8 - move platform survey tests out of `dillo`:
- Moved the platform adversarial integration test from `dillo/tests` to
  `dillo-devtree/tests`, matching ownership of `dillo-devtree::platform`.
- Removed the dillo overlay unit test that directly invoked platform survey;
  `/cpus` base-DTB rejection is now covered in `dillo-devtree`.
- Confirmed production dillo has no direct `Machine::survey` call; it receives
  the selected machine crate's `platform(dtb)` hook and carries the returned
  surveyed platform facts for resource and device orchestration.

Remaining divergence:
- `dillo` still carries the surveyed platform result while attaching resources
  and devices. This is expected until the orchestration surface shrinks further,
  but platform decoding and platform validation tests are no longer owned by
  dillo.

Local verification:
- `RUSTC_BOOTSTRAP=1 cargo fmt --all -- --check`
- `git diff --check`
- `RUSTC_BOOTSTRAP=1 CARGO_BUILD_RUSTFLAGS='-D warnings' cargo test -p dillo-devtree --test platform_adversarial`
...
Audit fix 3 - backend-owned guest-memory views:
- Removed the macOS/HVF `GuestMemoryMmap` builder from `dillo` and moved it to
  `dillo-machine-hvf::Vm::guest_memory()`, backed by the HVF-owned guest RAM
  mappings.
- Changed the Windows/WHP runner to pass only DTB/launch-derived GPA/size
  ranges into `dillo-machine-whp::Memory::from_ranges()`. The WHP backend now
  constructs the `GuestMemoryMmap` associated memory input and exposes the
  backend-owned memory view through `Vm::guest_memory()`.
...
Audit fix 10 - selected host services behind common trait:
- Added `dillo_machine::Host` for host-level services needed by the launcher:
  host architecture, raw terminal guard setup, panic terminal restoration, and
  supervisor signal watching.
- Implemented `Host` for KVM, HVF, and WHP `Vm`. Removed public backend
  `HOST_ARCH` and `platform()` helper APIs, and made the raw-stdio/signal
  helper methods private backend implementation details.
- Changed `dillo` to call only the common `Host` trait for these services.
  Backend-specific knowledge remains limited to selecting the `machine::Vm`
  type.
...
CI verification:
- `27225454458` passed on `cargo fmt`, `ubuntu-24.04`, `linux-arm64`,
  `macos-arm64`, and `windows-2025`.

Audit fix 18 - stop exporting backend interrupt internals:
- Made backend concrete interrupt-line/message-domain implementation types
  private: KVM aarch64 SPI and message-domain types, HVF SPI and GIC message
  domain types, and WHP IOAPIC interrupt-line type.
- Made the HVF `applevisor::VcpuHandle` import private. Backend shutdown still
  uses it internally, but it is no longer re-exported from `dillo-machine-hvf`.

Evidence:
- `grep -RInE "^pub use|pub type|pub struct|pub enum|pub trait|pub fn" dillo/deps/dillo-machine-kvm/src/lib.rs dillo/deps/dillo-machine-hvf/src/lib.rs dillo/deps/dillo-machine-whp/src/lib.rs`
  reports only expected backend associated-type surfaces: `Vm`, `Memory`,
  `CpuState`, `Cpu`, and `RawStdio`.
- `grep -RInE "(SpiInterruptLine|KvmMessageInterruptDomain|GicMessageInterruptDomain|IoApicInterruptLine|VcpuHandle)" dillo/src dillo/tests dillo/deps/dillo-machine/src --include='*.rs'`
  reports no launcher/common-trait caller dependencies.
...
- `CARGO_BUILD_RUSTFLAGS='-D warnings' cargo test --workspace --exclude snuffler`
- First local macOS boot run hit the known HVF `operation not allowed by the
  system` entitlement failure after one successful boot; immediate rerun of
  the same command passed.
- `CARGO_BUILD_RUSTFLAGS='-D warnings' cargo test -p dillo --features vm-tests --test boot -- --test-threads=1 --nocapture`
