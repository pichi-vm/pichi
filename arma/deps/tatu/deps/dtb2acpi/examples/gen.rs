...
fn main() {
    ...
    let oem = OemIdentity { oem_id: *b"PMIOVM", oem_table_id: *b"PMIOVMF ", oem_revision: 1, creator_id: *b"PMI ", creator_revision: 1 };
    let mut buf = Box::new(AcpiBuffer::<8192>::default());
    let n = buf.populate(&tree, &oem, base).expect("populate");
    std::io::stdout().write_all(&AsRef::<[u8]>::as_ref(&*buf)[..n]).expect("write");
}
